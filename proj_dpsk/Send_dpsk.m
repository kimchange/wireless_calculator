...
fun_binary_code_dpsk = dpskmod(fun_binary_code,2);

msg = zeros( 1,400+length(fun_binary_code_dpsk)*400);
...
msg(1:400) = 10*SIN1;
for ii = 2:(length(fun_binary_code_dpsk)+1)
    if(real(fun_binary_code_dpsk(ii-1)) == 1)
...
% 差分相干解调
y1 = voice_h(msg_begin+400:end).*voice_h(msg_begin:end-400);
y1 = filter(Lowpassvector,1,y1);

sample_t = 200;

msg_receive = double( (y1(sample_t:400: 400*118 +sample_t) < ...
    0.1*min(y1)));
